    ...
    return (
		<header className="flex items-center justify-between gap-4 p-4 pr-8 sticky top-0 bg-white z-50 sm:px-12 md:py-4 lg:px-24 lg:pr-28">
				...
				<ul className="flex flex-col gap-6 md:flex-row md:items-center">
					{categories.map(
						(category): JSX.Element => (
							<li key={category.id}>
								<Link
									className={`p-3 border-l-4 block transition-all ease-linear ${
										pathname === category.route
											? "font-medium bg-brand-light-purple/50 border-brand-purple lg:text-brand-purple"
											: "border-l-transparent hover:font-medium hover:bg-brand-light-purple/50 hover:border-brand-purple lg:hover:text-brand-purple lg:border-b-transparent"
									} lg:border-l-0 lg:border-b-2 lg:hover:bg-transparent lg:bg-transparent`}
									href={category.route}
								>
			...
			<ModalWrapper
				...
				toggleIsActive={setModalIsActive}
			/>
		</header>
	);
};
